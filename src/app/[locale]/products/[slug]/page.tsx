...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> { const { locale, slug } = await params; if (!isLocale(locale)) return {}; const product = await fixtureCatalogRepository.getProductBySlug(locale, slug); return product ? buildMetadata(locale, `/products/${slug}`, product.name, product.description) : {} }

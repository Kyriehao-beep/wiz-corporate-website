...
export function ProductDetailPage({ locale, product }: { locale: Locale; product: ProductDetail }) {
  ...
  return <main id="main-content">
    <section className="detail-hero"><Container className="detail-hero__grid"><div><p className="eyebrow">{t.productLabel} · {product.index}</p><h1>{product.name}</h1><p className="page-lead">{product.description}</p><ButtonLink href={buildRfqUrl({ locale, product: product.slug, source: 'product-detail' })}>{t.start}<ArrowRight aria-hidden="true" size={16}/></ButtonLink></div>
      {product.image ? (
        <div className="detail-image" aria-hidden="true"><Image alt={product.image.alt[locale]} fill sizes="(max-width: 900px) 100vw, 50vw" src={product.image.src} style={{ objectPosition: product.image.objectPosition }}/></div>
      ) : (
        <div className={`detail-patch detail-patch--${product.tone}`} aria-hidden="true"><span>W</span></div>
      )}
    </Container></section>
